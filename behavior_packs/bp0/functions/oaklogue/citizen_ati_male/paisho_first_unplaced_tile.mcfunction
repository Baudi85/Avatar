dialogue open @e[type=nom:ui_handler,c=1] @p citizen_ati_male.paisho_first_unplaced_tile
tag @s remove oaklogue.selector
tag @p add self
tag @e[tag=oaklogue.speaker.start,type=nom:speaker_citizen_ati_male,c=1] add speaker
execute @e[tag=speaker] ~ ~ ~ tag @e[tag=self] add has_speaker
execute @s[tag=!has_speaker] ~ ~ ~ tag @e[type=nom:speaker_citizen_ati_male,c=1] add speaker
