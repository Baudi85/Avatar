dialogue open @e[type=nom:ui_handler,c=1] @p varrick.greeting11
tag @s remove oaklogue.selector
tag @p add self
tag @e[tag=oaklogue.speaker.start,type=nom:npc_varrick,c=1] add speaker
execute @e[tag=speaker] ~ ~ ~ tag @e[tag=self] add has_speaker
execute @s[tag=!has_speaker] ~ ~ ~ tag @e[type=nom:npc_varrick,c=1] add speaker
